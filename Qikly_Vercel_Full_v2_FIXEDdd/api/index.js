...
async function buildShopQuote(itemsInput,couponCode){
  ...
  let discount=0,coupon=null;const code=clean(couponCode,40).toUpperCase();if(code){const coupons=await getShopCoupons();coupon=coupons.find(x=>String(x.code||"").toUpperCase()===code);if(!shopCouponValid(coupon,subtotal))throw new Error("Coupon is invalid, expired or minimum order value is not met.");if(coupon.type==="percent")discount=Math.min(Math.round(subtotal*num(coupon.value)/100),Math.max(0,money(coupon.maxDiscount)||subtotal));else if(coupon.type==="fixed")discount=Math.min(subtotal,Math.max(0,money(coupon.value)));}
  const settings=await getShopSettings();const afterDiscount=Math.max(0,subtotal-discount);const freeByCoupon=coupon?.type==="free_shipping";const shipping=freeByCoupon||afterDiscount>=money(settings.freeShippingAbove)?0:Math.max(0,money(settings.shippingFee));const total=afterDiscount+shipping;const codFee=Math.max(0,money(settings.codFee??shopDefaultSettings.codFee));
  return {items,subtotal,discount,shipping,total,coupon:coupon?{code:coupon.code,type:coupon.type,value:coupon.value}:null,freeShippingAbove:money(settings.freeShippingAbove),codEnabled:settings.codEnabled!==false,codFee,codTotal:total+codFee};
}
...
async function createShopOrder({quote,customer,address,userId,paymentMethod,paymentStatus,razorpayOrderId="",orderStatus="placed"}){
  const ref=db.collection("shopOrders").doc();const order={id:ref.id,orderNumber:shopOrderNumber(),userId:userId||"",customer:{name:clean(customer?.name,120),email:cleanEmail(customer?.email),phone:clean(customer?.phone,30)},address:{line1:clean(address?.line1,200),line2:clean(address?.line2,200),city:clean(address?.city,80),state:clean(address?.state,80),pincode:clean(address?.pincode,12),landmark:clean(address?.landmark,160)},items:quote.items,subtotal:quote.subtotal,discount:quote.discount,shipping:quote.shipping,codFee:paymentMethod==="cod"?Math.max(0,money(quote.codFee)):0,total:quote.total,coupon:quote.coupon,paymentMethod,paymentStatus,orderStatus,razorpayOrderId,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()};await db.runTransaction(async t=>{await decrementShopStock(t,quote.items,false);t.set(ref,order);});return jsonSafe(order);
}
app.post("/api/shop/visitor",async(req,res)=>{try{const ref=db.collection("shopAnalytics").doc("visitors");const today=new Intl.DateTimeFormat("en-CA",{timeZone:"Asia/Kolkata",year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date());await db.runTransaction(async t=>{const snap=await t.get(ref);const d=snap.exists?snap.data():{};const nextDate=String(d.date||"")===today?money(d.todayVisitors)+1:1;const total=money(d.totalVisitors)+1;t.set(ref,{date:today,todayVisitors:nextDate,totalVisitors:total,updatedAt:FieldValue.serverTimestamp()},{merge:true});});res.json({ok:true});}catch(e){res.status(500).json({error:e.message});}});
...
app.post("/api/shop/order/cod",async(req,res)=>{try{const baseQuote=await buildShopQuote(req.body.items,req.body.couponCode);if(!baseQuote.codEnabled)return res.status(400).json({error:"Cash on Delivery is currently disabled."});if(money(baseQuote.codFee)>0)return res.status(400).json({error:`COD requires an online COD fee of ${money(baseQuote.codFee)} before placing the order.`});const quote={...baseQuote,total:baseQuote.codTotal};const user=await currentUser(req);if(!clean(req.body.customer?.name,120)||!clean(req.body.customer?.phone,30)||!clean(req.body.address?.line1,200)||!clean(req.body.address?.city,80)||!clean(req.body.address?.state,80)||!clean(req.body.address?.pincode,12))return res.status(400).json({error:"Please complete delivery details."});const order=await createShopOrder({quote,customer:req.body.customer,address:req.body.address,userId:user?.id||"",paymentMethod:"cod",paymentStatus:"pending",orderStatus:"placed"});await incrementShopCoupon(quote.coupon?.code);res.json({ok:true,order});}catch(e){res.status(400).json({error:e.message||"Unable to place COD order."});}});

app.post("/api/shop/order/cod-fee/create",async(req,res)=>{try{const baseQuote=await buildShopQuote(req.body.items,req.body.couponCode);if(!baseQuote.codEnabled)return res.status(400).json({error:"Cash on Delivery is currently disabled."});const quote={...baseQuote,total:baseQuote.codTotal};const codFee=Math.max(0,money(baseQuote.codFee));const customer={name:clean(req.body.customer?.name,120),email:cleanEmail(req.body.customer?.email),phone:clean(req.body.customer?.phone,30)};const address={line1:clean(req.body.address?.line1,200),line2:clean(req.body.address?.line2,200),city:clean(req.body.address?.city,80),state:clean(req.body.address?.state,80),pincode:clean(req.body.address?.pincode,12),landmark:clean(req.body.address?.landmark,160)};if(!customer.name||!customer.phone||!address.line1||!address.city||!address.state||!address.pincode)return res.status(400).json({error:"Please complete delivery details."});const user=await currentUser(req);if(codFee<=0){const order=await createShopOrder({quote,customer,address,userId:user?.id||"",paymentMethod:"cod",paymentStatus:"pending",orderStatus:"placed"});await incrementShopCoupon(quote.coupon?.code);return res.json({ok:true,direct:true,order});}const rpayKey=String(process.env.RAZORPAY_KEY_ID||"");const rpaySecret=String(process.env.RAZORPAY_KEY_SECRET||"");if(!rpayKey||!rpaySecret)return res.status(503).json({error:"Online payment is not configured for the COD fee."});const ref=db.collection("shopOrders").doc();const receipt=`codfee_${Date.now().toString().slice(-10)}_${crypto.randomBytes(2).toString("hex")}`;const rp=await razorpay.orders.create({amount:codFee*100,currency:"INR",receipt,notes:{shopOrderId:ref.id,paymentType:"cod_fee"}});const order={id:ref.id,orderNumber:shopOrderNumber(),userId:user?.id||"",customer,address,items:quote.items,subtotal:quote.subtotal,discount:quote.discount,shipping:quote.shipping,codFee,total:quote.total,codAmountDue:Math.max(0,quote.total-codFee),coupon:quote.coupon,paymentMethod:"cod",paymentStatus:"cod_fee_pending",orderStatus:"payment_pending",razorpayOrderId:rp.id,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()};await ref.set(order);res.json({ok:true,keyId:rpayKey,razorpayOrder:{id:rp.id,amount:rp.amount,currency:rp.currency},order:jsonSafe(order)});}catch(e){res.status(400).json({error:e.message||"Unable to start COD fee payment."});}});

app.post("/api/shop/order/cod-fee/verify",async(req,res)=>{try{const {razorpay_order_id,razorpay_payment_id,razorpay_signature}=req.body||{};if(!razorpay_order_id||!razorpay_payment_id||!razorpay_signature)return res.status(400).json({error:"COD fee payment details are incomplete."});const expected=crypto.createHmac("sha256",String(process.env.RAZORPAY_KEY_SECRET||"")).update(`${razorpay_order_id}|${razorpay_payment_id}`).digest("hex");if(!timingSafeEqualText(expected,razorpay_signature))return res.status(400).json({error:"COD fee payment verification failed."});const snap=await db.collection("shopOrders").where("razorpayOrderId","==",razorpay_order_id).limit(1).get();if(snap.empty)return res.status(404).json({error:"COD order not found."});const orderRef=snap.docs[0].ref;const existing=snap.docs[0].data();if(existing.paymentMethod!=="cod")return res.status(400).json({error:"Invalid COD payment session."});if(existing.paymentStatus==="cod_fee_paid")return res.json({ok:true,order:jsonSafe({id:orderRef.id,...existing})});const payment=await razorpay.payments.fetch(razorpay_payment_id);if(payment.status!=="captured"||money(payment.amount)!==money(existing.codFee)*100)return res.status(400).json({error:"COD fee payment amount/status is not valid."});await db.runTransaction(async t=>{const latest=await t.get(orderRef);const o=latest.data()||{};if(o.paymentStatus==="cod_fee_paid")return;await decrementShopStock(t,o.items||[],false);t.set(orderRef,{paymentStatus:"cod_fee_paid",orderStatus:"placed",codFeePaid:true,razorpayPaymentId:razorpay_payment_id,razorpaySignature:razorpay_signature,updatedAt:FieldValue.serverTimestamp()},{merge:true});});const updated=await orderRef.get();await incrementShopCoupon(updated.data()?.coupon?.code);res.json({ok:true,order:jsonSafe({id:updated.id,...updated.data()})});}catch(e){res.status(400).json({error:e.message||"COD fee payment verification failed."});}});
...
app.post("/api/admin/shop/coupon",guardAdmin,async(req,res)=>{try{const b=req.body||{},id=clean(b.id,80)||clean(b.code,40).toUpperCase();const code=clean(b.code,40).toUpperCase();const type=["fixed","free_shipping"].includes(b.type)?b.type:"percent";const value=Math.max(0,money(b.value));if(!code||(type!=="free_shipping"&&value<=0))return res.status(400).json({error:"Coupon code and valid value are required."});const payload={code,type,value,minOrder:Math.max(0,money(b.minOrder)),maxDiscount:Math.max(0,money(b.maxDiscount)),active:b.active!==false,expiry:clean(b.expiry,80),usageLimit:Math.max(0,money(b.usageLimit)),updatedAt:FieldValue.serverTimestamp()};await db.collection("shopCoupons").doc(id).set(payload,{merge:true});res.json({ok:true,id});}catch(e){res.status(500).json({error:e.message});}});
