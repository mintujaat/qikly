...
async function buildShopQuote(itemsInput,couponCode){
  ...
  const settings=await getShopSettings();const afterDiscount=Math.max(0,subtotal-discount);const freeByCoupon=coupon?.type==="free_shipping";const shipping=freeByCoupon||afterDiscount>=money(settings.freeShippingAbove)?0:Math.max(0,money(settings.shippingFee));const total=afterDiscount+shipping;
