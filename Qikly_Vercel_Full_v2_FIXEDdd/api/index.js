...
app.post("/api/shop/orders/:id/cancel",guardUser,async(req,res)=>{try{const ref=db.collection("shopOrders").doc(clean(req.params.id,120));await db.runTransaction(async t=>{const snap=await t.get(ref);if(!snap.exists||snap.data().userId!==req.user.id)throw new Error("Order not found.");const o=snap.data();if(!["payment_pending","placed","packed"].includes(o.orderStatus))throw new Error("This order can no longer be cancelled.");const reserved=o.paymentMethod==="cod"||o.paymentStatus==="paid";if(reserved)await decrementShopStock(t,o.items||[],true);t.set(ref,{orderStatus:"cancelled",paymentStatus:o.paymentStatus==="paid"?"refund_pending":o.paymentStatus,updatedAt:FieldValue.serverTimestamp()},{merge:true});});res.json({ok:true});}catch(e){res.status(400).json({error:e.message});}});
