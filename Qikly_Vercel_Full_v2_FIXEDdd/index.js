...
function createMemoryDocRef(collectionName, rawId, list, preloadedEntry = null) {
  const docId = String(rawId ?? `doc_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`)
  let entry = preloadedEntry ?? list.find(item => String(item.id) === docId) ?? { id: docId }
  const sync = (nextEntry, mergeMode = false) => {
    const merged = mergeMode && entry ? { ...entry, ...nextEntry } : { ...(nextEntry || {}), id: docId }
    entry = merged
    const index = list.findIndex(item => String(item.id) === docId)
  ...
  const docRef = {
    id: docId,
    get: async () => ({ exists: !!entry, data: () => entry ? { ...entry } : null, id: docId, ref: docRef }),
    set: async (nextData, options = {}) => sync(nextData || {}, options.merge === true),
    update: async (patch) => sync(patch || {}, true),
    delete: async () => {
      const index = list.findIndex(item => String(item.id) === docId)
    ...
    collection: (subCollectionName) => {
      const nestedKey = `${collectionName}.${docId}.${subCollectionName}`
      if (!inMemoryCollections[nestedKey]) inMemoryCollections[nestedKey] = []
      const nestedList = inMemoryCollections[nestedKey]
      return {
        doc: (subId) => createMemoryDocRef(nestedKey, subId, nestedList),
        get: async () => ({
          docs: nestedList.map(item => ({ id: item.id, data: () => ({ ...item }), ref: createMemoryDocRef(nestedKey, item.id, nestedList, item) })),
          empty: nestedList.length === 0
        })
      }
    }
  }

  if (preloadedEntry && !list.some(item => String(item.id) === docId)) list.push(preloadedEntry)
  else if (!list.some(item => String(item.id) === docId)) list.push(entry)
  return docRef
}

const memoryDb = {
  collection(collectionName) {
    if (!inMemoryCollections[collectionName]) inMemoryCollections[collectionName] = []
    const list = inMemoryCollections[collectionName]
    return {
      doc: (docId) => createMemoryDocRef(collectionName, docId, list),
      where: (field, operator, value) => ({
        limit: (limitCount) => ({
          get: async () => {
            ...
            return {
              docs: filtered.map(item => ({ id: item.id, data: () => ({ ...item }), ref: createMemoryDocRef(collectionName, item.id, list, item) })),
      ...
      limit: (limitCount) => ({
        get: async () => ({
          docs: list.slice(0, limitCount).map(item => ({ id: item.id, data: () => ({ ...item }), ref: createMemoryDocRef(collectionName, item.id, list, item) })),
          empty: list.length === 0
        })
      })
    }
  ...
  runTransaction: async (callback) => callback({
    ...
    update: async (docRef, patch) => docRef.update(patch)
  })
}
...
function getDb() {
  ...
  try {
    ...
    throw new Error('FIREBASE_SERVICE_ACCOUNT_JSON is not valid JSON. Paste the complete Firebase service-account JSON.')
  }
  if (!serviceAccount?.client_email || !serviceAccount?.private_key) {
    throw new Error('FIREBASE_SERVICE_ACCOUNT_JSON is incomplete. client_email and private_key are required.')
  }
...
const FieldValue = admin.firestore?.FieldValue || {
  ...
  increment: (value = 1) => ({ __op: 'increment', value }),
  delete: () => ({ __op: 'delete' })
}
...
const inMemoryCollections = {
  settings: [{ id: 'main', ...defaults.settings }],
  chatbot: [{ id: 'main', ...defaults.chatbot }],
...
app.post('/api/shop/create-order', guardUser, async (req, res) => {
  try {
    ...
    const rpOrder = await getRazorpay().orders.create({ amount: t.total * 100, currency: 'INR', receipt: `shop_${Date.now()}_${crypto.randomBytes(3).toString('hex')}`, notes: { uid: req.user.id, orderId: ref.id } })
...
app.post('/api/shop/verify-payment', guardUser, async (req, res) => {
  try {
    ...
    const payment = await getRazorpay().payments.fetch(razorpay_payment_id); if (payment.status !== 'captured' || money(payment.amount) !== money(o.total) * 100 || payment.order_id !== razorpay_order_id) return res.status(400).json({ error: 'Payment is not valid or captured.' })
...
if (require.main === module) {
  ...
  app.listen(port, () => {
    console.log(`Qikly app listening on http://localhost:${port}`)
  })
}
